...
import { ADMIN_LESSONS_GET_LIST } from '../slice';
...
function* getList(action: any) {
  ...
  yield call(
    handleApiCall,
    action,
    ADMIN_LESSONS_GET_LIST,
    api.getAllLessons.bind(api),
  );
}

export function* adminLessonsSaga() {
  yield takeLatest(ADMIN_LESSONS_GET_LIST.TRIGGER, getList);
}
