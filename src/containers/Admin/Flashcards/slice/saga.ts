...
import { ADMIN_FLASHCARDS_GET_LIST } from '../slice';
...
function* getList(action: any) {
  ...
  yield call(
    handleApiCall,
    action,
    ADMIN_FLASHCARDS_GET_LIST,
    api.getAllFlashcardSets.bind(api),
  );
}

export function* adminFlashcardsSaga() {
  yield takeLatest(ADMIN_FLASHCARDS_GET_LIST.TRIGGER, getList);
}
