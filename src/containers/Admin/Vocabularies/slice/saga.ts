...
import { ADMIN_VOCABULARIES_GET_LIST } from '../slice';
import { vocabulariesApi } from 'services';

const api = vocabulariesApi;
...
function* getList(action: any) {
  ...
  yield call(
    ...
    api.getAllVocabularies.bind(api),
  );
}

export function* adminVocabulariesSaga() {
  yield takeLatest(ADMIN_VOCABULARIES_GET_LIST.TRIGGER, getList);
}
